...
export const getProducts = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  ...
  // Aggregate stats from the whole collection
  ...
  const stats = statsData[0] || { totalInventoryValue: 0, lowStockCount: 0, activeProductsCount: 0 };
...
export const createProduct = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  ...
  // Sync: if only price is sent, mirror it to sellingPrice (canonical selling price field)
...
export const updateProduct = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  ...
  // Sync: keep price and sellingPrice in sync (canonical = sellingPrice)
  if (bodyData.price && !bodyData.sellingPrice) {
    bodyData.sellingPrice = bodyData.price;
